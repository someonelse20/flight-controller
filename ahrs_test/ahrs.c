...
vector_t mag_rejection(ahrs_t *ahrs, vector_t mag) {
  ...
  if (mag_distortion > ahrs->settings.min_mag_distortion &&
      mag_distortion < ahrs->settings.max_mag_distoriton) {
...
vector_t accel_rejection(ahrs_t *ahrs, vector_t accel) {
  // calculate the amount of time that the accelerometer measurment is
  // unreliable
  if (ahrs->accel_t_timestamp == 0)
    ahrs->accel_t_timestamp = get_timestamp(1);
  if ((vector_norm(accel) - 1) > -ahrs->settings.accel_rejection &&
      (vector_norm(accel) - 1) < ahrs->settings.accel_rejection)
    ahrs->accel_t = 0;
  else
...
void initialize(ahrs_t *ahrs) {
  ...
  if (!ahrs->initialized)
    ahrs->gain = ahrs->settings.gain_normal +
                 (pow(ahrs->settings.init_time, -current_time(*ahrs)) /
                  ahrs->settings.init_time) *
                     (ahrs->settings.gain_init - ahrs->settings.gain_normal);
  else
    ahrs->gain = ahrs->settings.gain_normal;
}

void update(ahrs_t *ahrs, vector_t gyro, vector_t accel, vector_t mag,
            const float DT) {
  // set timestamp
  ...
  // calibrate sensors
  vector_t gyro_calibrated =
      calibrate_gyro_accel(gyro, ahrs->settings.gyro_calibrate.rotation_matrix,
                           ahrs->settings.gyro_calibrate.sensitivity,
                           ahrs->settings.gyro_calibrate.bias);
  vector_t accel_calibrated = calibrate_gyro_accel(
      accel, ahrs->settings.accel_calibrate.rotation_matrix,
      ahrs->settings.accel_calibrate.sensitivity,
      ahrs->settings.accel_calibrate.bias);
  vector_t mag_calibrated =
      calibrate_mag(mag, ahrs->settings.mag_calibrate.rotation_matrix,
  ...
  ahrs->a_error = cross_product(
      accel_normalized,
      (vector_t){(2 * ahrs->orientation.x * ahrs->orientation.z) -
  ...
  vector_t cross_a_m =
      cross_product(accel_normalized, normalize_vector(mag_calibrated));
  ahrs->m_error = cross_product(
      cross_a_m,
      (vector_t){(-2 * pow(ahrs->orientation.w, 2)) + 1 -
                     (2 * pow(ahrs->orientation.x, 2)),
                 (-2 * ahrs->orientation.x * ahrs->orientation.y) +
                     (2 * ahrs->orientation.w * ahrs->orientation.z),
  ...
  if (vector_norm(accel_conditioned) > 0 && vector_norm(mag_conditioned) > 0)
    ...
    ahrs->error = (vector_t){0, 0, 0};

  // complementary filter
  vector_t gyro_error_product =
      subtract_vector(gyro_conditioned, scale_vector(ahrs->error, ahrs->gain));

  quaternion_t orientation_rate_of_chage = quaternion_product(
      scale_quaternion(ahrs->orientation, 0.5),
      (quaternion_t){0, gyro_error_product.x, gyro_error_product.y,
                     gyro_error_product.z});

  quaternion_t orientation_unnormalised = add_quaternion(
      ahrs->orientation, scale_quaternion(orientation_rate_of_chage, DT));
  ...
  if (ahrs->settings.add_declination) {
    quaternion_t declination = euler_to_quaternion(
        (vector_t){0, 0, ahrs->settings.declination * (M_PI / 180)});
    orientation_declination =
        quaternion_product(ahrs->orientation, declination);
  }

  quaternion_t orientation_global_declination =
      quaternion_conjugate(orientation_declination);
...
vector_t acceleration_zero(ahrs_t ahrs) {
  return add_vector(ahrs.accel_calibrated,
                    (vector_t){2 * ahrs.orientation.x * ahrs.orientation.z -
                                   2 * ahrs.orientation.w * ahrs.orientation.y,
                               2 * ahrs.orientation.y * ahrs.orientation.z +
                                   2 * ahrs.orientation.w * ahrs.orientation.x,
...
vector_t acceleration_global(ahrs_t ahrs) {
  ...
  quaternion_t acceleration_global_quaternion = quaternion_product(
      quaternion_product(
          ahrs.orientation,
          (quaternion_t){0, accel_zero.x, accel_zero.y, accel_zero.z}),
      ahrs.orientation);

  return (vector_t){acceleration_global_quaternion.x,
